...
def _parse_task_from_string(input_string: str):
    ...
    Args:
    input_string:  An input string to test the regex_pattern against.  Defaulted if not provided
    ...
    if regex_match is not None:
        ...
        markdown_part = regex_match.group(1)  # We avoid strip here to maintain indentation.
        task_part = regex_match.group(2).strip()

        # Remove #hashtags from the string.
        task_part = task_part.replace('#', '').strip()
...
def get_omnifocus_front_matter_setting(input_string:str):
    """
    Parses the omnifocus property from the frontmatter of a string or file that that string is the path for

    This is used to disallow specific notes (e.g. Packing list Template) from having parsed To-do's migrated to omnifocus

    Args:
        input_string: A fully qualified file path or a string

    Returns: A Boolean flag, defaulting to True where no setting is found.
    Raises: Value error if the 'omnifocus' key is supplied in the frontmatter with any value other than 'true' or 'false'
    """
    ...
    omnifocus = metadata.get('omnifocus')

    if omnifocus is None:
        ret_val = True
    elif omnifocus is False:
        ret_val = False  # Expected when frontmatter explicitly contains 'omnifocus: false'
    elif omnifocus is True:
        ret_val = True   # Not necessary to supply, but this is when the frontmatter explicitly contains 'omnifocus: true'
    else:
        raise ValueError(f"Could not parse 'omnifocus' setting front matter.  There is no handling.  "
                         f"Got value '{omnifocus}', which is of type {type(omnifocus)} from the input: \n {input_string}")
    return ret_val
